...
- Output the JSON to a file or code block named `classified-test-issues-part1.json`
- After confirming that the output looks correct, we will continue with the next batch (e.g., 5 more issues per step)

Only respond with the JSON output or file contents—no extra explanation.