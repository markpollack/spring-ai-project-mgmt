...
```bash
# Option A: Use the fetch_labels.sh script (recommended)
./fetch_labels.sh
# This creates labels.json automatically

# Option B: Manual extraction with gh CLI
...
### Step 2.2: Clean Label List (IMPORTANT)
Edit `labels.json` to remove problematic labels. This is just a simple JSON array of label names:
...
[
  ...
  "streaming",
  "Chat Memory"
  // ... other technical labels
]
```

**Remove these categories from labels.json:**
...
**Keep only technical/objective labels.**

### Step 2.3: Data Flow Summary
**Input Files:**
- `issues/analysis/issues_analysis_ready.json` - From Phase 1 data collection
- `labels.json` - Cleaned label list (created in Step 2.1-2.2)

**Note:** The label mapping files (github-labels-mapping.json) are NOT used by stratified_split.py. The script has built-in label grouping logic:
- Groups vector store variants (pinecone, qdrant, etc.) → "vector store"
- Groups model clients (openai, claude, ollama, etc.) → "model client"
- Groups data stores (mongo, cassandra, etc.) → "data store"
...
```
Phase 1 Output:
  issues/analysis/issues_analysis_ready.json (all issues with labels)
           +
Phase 2 Output:
  labels.json (cleaned label list)
    ...
    stratified_split.py
           ↓
Phase 3 Output:
  issues/stratified_split/train_set.json (80%)
  issues/stratified_split/test_set.json (20%)
           ↓
    process_full_classification.py
           ↓
  test_issues_for_classification.json (for LLM classification)
```
