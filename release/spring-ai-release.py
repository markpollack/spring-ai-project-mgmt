#!/usr/bin/env python3
"""
Spring AI Release Script - Unified Release Automation

Automates all Spring AI release types with enhanced safety and interactivity.
Supports semantic versioning: PATCH, MINOR releases, Milestones, and Release Candidates.
Based on proven patterns from pr-review scripts.

Release Types Supported:
- PATCH (1.0.1, 1.0.2): Backward compatible bug fixes (from maintenance branch)
- MINOR (1.1.0): Backward compatible new features (from main branch)  
- Milestones (1.1.0-M1, M2, M3): Minor version prereleases (from main branch)
- Release Candidates (1.1.0-RC1): Minor version prereleases (from main branch)

Usage:
    python3 spring-ai-release.py 1.0.1 --dry-run           # Patch release
    python3 spring-ai-release.py 1.1.0-M1 --dry-run        # Milestone release
    python3 spring-ai-release.py 1.1.0-RC1 --dry-run       # Release candidate
    python3 spring-ai-release.py 1.1.0 --dry-run           # Minor release
"""
...
@dataclass
class ReleaseConfig:
    """Configuration for the unified release workflow"""
    script_dir: Path
    target_version: str
    branch: Optional[str] = None  # Will auto-detect if None
    ...
    def next_dev_version(self) -> str:
        ...
        if release_type == 'milestone':
            ...
            return f"{major}.{next_minor}.0-SNAPSHOT"
    ...
    def should_skip_post_maven_workflow(self) -> bool:
        ...
        release_type = self.get_release_type()
        # Milestones are standalone releases, skip development version updates
...
class ReleaseWorkflow:
    ...
    def display_summary(self):
        ...
        Logger.bold("\n" + "="*60)
        Logger.bold("SPRING AI RELEASE SUMMARY")
        Logger.bold("="*60)
        Logger.info(f"Release Type: {self.config.get_release_type().upper()}")
        Logger.info(f"Target Version: {self.config.target_version}")
    ...
    def execute_post_maven_central(self) -> bool:
        ...
        # Check if this release type should skip post-Maven steps
        if self.config.should_skip_post_maven_workflow():
            release_type = self.config.get_release_type()
            Logger.info(f"🎯 {release_type.capitalize()} release detected - skipping development version setup")
            Logger.success("✅ Milestone/prerelease workflow complete!")
            Logger.info("💡 Milestone releases don't require development version updates")
            return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Spring AI Release Script - Unified automation for PATCH, MINOR, Milestone, and RC releases",
...
Examples:
  ...
  # Release candidates (auto-detects main branch) 
  ...
  # Manual branch override (if needed)
  python3 spring-ai-release.py 1.1.0-M1 --branch main --dry-run
  
  # Post-Maven Central workflow (not needed for milestones)
...
Skip-to options: setup, set-version, build, commit-release, tag, push, docs, javadoc, maven-central
    ...
    parser.add_argument('--branch', help='Branch to release from (auto-detected by default)')
