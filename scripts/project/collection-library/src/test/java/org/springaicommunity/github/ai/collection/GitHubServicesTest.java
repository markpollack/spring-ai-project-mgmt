...
/**
 * Tests for GitHubServices using plain JUnit with mocked APIs.
...
class GitHubServicesTest {
    ...
    class GitHubRestServiceTest {
        ...
        void shouldGetRepositoryInfoViaRestAPI() {
            ...
            when(mockRequestHeadersUriSpec.uri(anyString(), anyString(), anyString())).thenReturn(mockRequestHeadersUriSpec);
        ...
        class SearchQueryBuildingTest {
            ...
            void shouldBuildBasicSearchQuery() {
                String query = gitHubRestService.buildSearchQuery("spring-projects", "spring-ai", "closed", null, "any");
    ...
    class GitHubGraphQLServiceTest {
        ...
        void shouldGetSearchIssueCountViaGraphQL() {
            ...
            int result = gitHubGraphQLService.getSearchIssueCount("repo:spring-projects/spring-ai is:issue is:closed label:\"bug\"");
        ...
        @ParameterizedTest
        @CsvSource({
            "spring-projects, spring-ai, open, 1000",
            "microsoft, vscode, closed, 5000",
            "kubernetes, kubernetes, all, 10000"
        })
        @DisplayName("Should handle different repository and state combinations")
        void shouldHandleDifferentRepositoryAndStateCombinations(String owner, String repo, String state, int expectedCount) {
            String mockResponse = String.format("{\"data\":{\"repository\":{\"issues\":{\"totalCount\":%d}}}}", expectedCount);
    ...
    class JsonNodeUtilsTest {
        ...
        void shouldReturnEmptyForInvalidDatetimeFormat() {
            Optional<LocalDateTime> result = jsonNodeUtils.getDateTime(testNode, "user", "name"); // Name is not a datetime
